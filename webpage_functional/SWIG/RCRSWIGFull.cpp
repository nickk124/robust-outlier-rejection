...
// handles the request for the functionalForm. fType is an int corresponding to a certain function type. takes in (C?) arrays that are then converted to usable cpp vectors. rejection technique specified by corresponding int.
// rejTechNo =
// 1 for SS_MEDIAN_DL
// 2 for LS_MODE_68
// 3 for LS_MODE_DL
...
// g++ -std=c++11 -shared -Wl,-soname,RCRControllerFull -o RCRControllerFull.so -fPIC RCRControllerFull.cpp
std::vector <double> requestHandlerUnWeighted(std::vector <double> x, std::vector <double> y, std::vector <double> sigma_y, std::vector <double> guess, int fType, int dataSize, int rejTechNo)
{
  ...
  */
    ...
    */
		...
		FunctionalForm model = FunctionalForm(function_linear, x, y, sigma_y, partialsvector_linear, tolerance, guess);
		RCR rcr = RCR(rejTech);
	...
	if (fType == 2) { // quadratic
		...
		// performs functionalRCR:
		FunctionalForm model = FunctionalForm(function_quadratic, x, y, sigma_y, partialsvector_quadratic, tolerance, guess);
		RCR rcr = RCR(rejTech);
		rcr.setParametricModel(model);
		rcr.performBulkRejection(y);
	...
	if (fType == 3) { // cubic
		...
		// performs functionalRCR:
		FunctionalForm model = FunctionalForm(function_cubic, x, y, sigma_y, partialsvector_cubic, tolerance, guess);
		RCR rcr = RCR(rejTech);
		rcr.setParametricModel(model);
		rcr.performBulkRejection(y);
	...
	if (fType == 4) { // power law
		...
		// performs functionalRCR:
		FunctionalForm model = FunctionalForm(function_powerlaw, x, y, sigma_y, partialsvector_powerlaw, tolerance, guess);
		RCR rcr = RCR(rejTech);
		rcr.setParametricModel(model);
		rcr.performBulkRejection(y);
	...
	if (fType == 5) { // exponential
		...
		// performs functionalRCR:
		FunctionalForm model = FunctionalForm(function_exponential, x, y, sigma_y, partialsvector_exponential, tolerance, guess);
		RCR rcr = RCR(rejTech);
		rcr.setParametricModel(model);
		rcr.performBulkRejection(y);
	...
	if (fType == 6) { // logarithmic
		...
		// performs functionalRCR:
		FunctionalForm model = FunctionalForm(function_logarithmic, x, y, sigma_y, partialsvector_logarithmic, tolerance, guess);
		RCR rcr = RCR(rejTech);
		rcr.setParametricModel(model);
		rcr.performBulkRejection(y);
...
std::vector <double> requestHandlerWeighted(std::vector <double> x, std::vector <double> y, std::vector <double> sigma_y, std::vector <double> guess, std::vector <double> w, int fType, int dataSize, int rejTechNo)// handles the request for the functionalForm. fType is an character corresponding to a certain function type. takes in (C?) arrays that are then converted to usable cpp vectors.
{
	//tolerance:
	double tolerance = 0.01;
  ...
  */
	...
	if (fType == 1) { // linear
		...
		// performs functionalRCR:
		FunctionalForm model = FunctionalForm(function_linear, x, y, sigma_y, partialsvector_linear, tolerance, guess, w);
		RCR rcr = RCR(rejTech);
		rcr.setParametricModel(model);
	...
	if (fType == 2) { // quadratic
		...
		// performs functionalRCR:
		FunctionalForm model = FunctionalForm(function_quadratic, x, y, sigma_y, partialsvector_quadratic, tolerance, guess, w);
		RCR rcr = RCR(rejTech);
		rcr.setParametricModel(model);
		rcr.performBulkRejection(w, y);
	...
	if (fType == 3) { // cubic
		...
		// performs functionalRCR:
		FunctionalForm model = FunctionalForm(function_cubic, x, y, sigma_y, partialsvector_cubic, tolerance, guess, w);
		RCR rcr = RCR(rejTech);
		rcr.setParametricModel(model);
		rcr.performBulkRejection(w, y);
	...
	if (fType == 4) { // power law
		...
		// performs functionalRCR:
		FunctionalForm model = FunctionalForm(function_powerlaw, x, y, sigma_y, partialsvector_powerlaw, tolerance, guess, w);
		RCR rcr = RCR(rejTech);
		rcr.setParametricModel(model);
		rcr.performBulkRejection(w, y);
	...
	if (fType == 5) { // exponential
		...
		// performs functionalRCR:
		FunctionalForm model = FunctionalForm(function_exponential, x, y, sigma_y, partialsvector_exponential, tolerance, guess, w);
		RCR rcr = RCR(rejTech);
		rcr.setParametricModel(model);
		rcr.performBulkRejection(w, y);
	...
	if (fType == 6) { // logarithmic
		...
		// performs functionalRCR:
		FunctionalForm model = FunctionalForm(function_logarithmic, x, y, sigma_y, partialsvector_logarithmic, tolerance, guess, w);
		RCR rcr = RCR(rejTech);
		rcr.setParametricModel(model);
		rcr.performBulkRejection(w, y);
